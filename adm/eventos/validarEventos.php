    ...
    <div class="aaa" style="height: 100%;/* flex: auto; */display: flex; flex-direction: row;">
        <div class="d-flex flex-column flex-shrink-0 p-3 bg-light navCor" style="width: 280px; height: 100vh;" id="sidebar-admin">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li>
                    <a href="eventos/validarEventos.php" class="nav-link ativado">
                        Eventos
                    </a>
                </li>
                <li>
                    <a href="../comunidades/validarComunidades.php" class="nav-link naoAtivado">
                        Comunidades
                    </a>
                </li>
            </ul>
        ...
        <div class="card" style="width: 170vh;">
            ...
            <div class="qntd">
                <?php
                $sqlCount = "SELECT COUNT(*) AS qntdNaoAprov FROM Eventos WHERE aprovado = 0";
                $conexao = obterConexao();
                $resultadoCount = mysqli_query($conexao, $sqlCount);
                $qntdNaoAprov = mysqli_fetch_assoc($resultadoCount); ?>
                <h5 class="msgQntd">Qntd de Eventos pendentes de avaliação = <?php echo $qntdNaoAprov['qntdNaoAprov']; ?></h5>
            </div>
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT e.id_evento, e.nome_evento, p.nome_ponto, c.nome_categoria, DATE_FORMAT(e.data_evento, '%d/%m/%Y') AS data_formatada, e.aprovado
                    FROM Eventos e
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td name='id'>" . $row['id_evento'] . "</td>";
                        echo "<td>" . $row['nome_evento'] . "</td>";
                        echo "<td>" . $row['nome_ponto'] . "</td>";
                        echo "<td>" . $row['nome_categoria'] . "</td>";
                        echo "<td>" . $row['data_formatada'] . "</td>";
                        echo "<td>" . $aprovado . "</td>";
                        echo "<td><a class='btn btn-sm buttonVer' href='verEvento.php?id_evento=" . $row["id_evento"] . "&nome=" . $row['nome_evento'] . "'>Ver mais</a> </td></tr>";
                    }
                    ?>
                </tbody>
            </table>
