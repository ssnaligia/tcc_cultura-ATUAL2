    ...
    <div class="aaa" style="height: 100%;/* flex: auto; */display: flex; flex-direction: row;">
        <div class="d-flex flex-column flex-shrink-0 p-3 bg-light navCor" style="width: 280px; height: 100vh;" id="sidebar-admin">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                <li>
                    <a href="validarPontos.php" class="nav-link ativado" aria-current="page">
                        Pontos Culturais
                    </a>
                </li>
                <li>
        ...
        <div class="card" style="width: 170vh;">
            ...
            <div class="qntd">
                <?php
                $sqlCount = "SELECT COUNT(*) AS qntdNaoAprov FROM PontosCulturais WHERE aprovado = 0";
                $conexao = obterConexao();
                $resultadoCount = mysqli_query($conexao, $sqlCount);
                $qntdNaoAprov = mysqli_fetch_assoc($resultadoCount); ?>
                <h5 class="msgQntd">Qntd de Pontos pendentes de avaliação = <?php echo $qntdNaoAprov['qntdNaoAprov']; ?></h5>
            </div>
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT p.id_ponto, p.nome_ponto, c.nome_categoria, p.aprovado FROM PontosCulturais p
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td name='id'>" . $row['id_ponto'] . "</td>";
                        echo "<td>" . $row['nome_ponto'] . "</td>";
                        echo "<td>" . $row['nome_categoria'] . "</td>";
                        echo "<td>" . $aprovado . "</td>";
                        echo "<td><a class='btn btn-sm buttonVer' href='verPonto.php?id_ponto=" . $row["id_ponto"] . "&nome=" . $row['nome_ponto'] . "'>Ver mais</a> </td>
            </tr>";
                    }
                    ?>
                </tbody>
